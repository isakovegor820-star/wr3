    ...
    for case in cases:
        record = await service.create_audit(
            CreateAuditRequest(
                ...
                source=case["source"],
                requested_depth=case.get("requested_depth", "standard"),
        ...
        latest = poc_runs[-1] if poc_runs else None
        results.append(
            {
                "id": case["id"],
                "audit_id": str(record.audit_id),
                "state": record.state,
                "poc_status": latest.status if latest else "missing",
                "poc_error": latest.error if latest else "missing_foundry_poc_run",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run wr3 local Foundry PoC fixture pass.")
    parser.add_argument("--fixtures", type=Path, default=ROOT / "benchmarks/fixtures/poc_cases.json")
    parser.add_argument("--out", type=Path, default=ROOT / "artifacts/poc/local-poc-report.json")
