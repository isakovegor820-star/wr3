...
test("dismissed findings do not penalize score", () => {
  const score = scoreAudit([finding({ exploitability: "dismissed", confidence: 1 })]);
  assert.equal(score.code_security_score, 100);
});
