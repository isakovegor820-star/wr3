    ...
    def integrations(self) -> list[IntegrationDescriptor]:
        ...
        return [
            ...
            IntegrationDescriptor(
                ...
                next_step="Для бесплатного тарифа держать выключенным; включать только для paid-тестов и ZDR-маршрута.",
            ...
            IntegrationDescriptor(
                id="payments_manual_usdc",
                label="Manual USDC",
                priority="P1",
                category="billing",
                status="manual" if s.usdc_receive_address else "disabled",
                free_mode="API не требуется; подтверждение вручную.",
                used_by=["billing mock", "активация подписки ревьюером"],
                env_vars=["WR3_USDC_RECEIVE_ADDRESS"],
                next_step="Добавлять receive address только при приёме paid beta users.",
                notes=["No Telegram Stars dependency."],
            ),
            IntegrationDescriptor(
                id="checkout_providers",
                label="Request Finance / Polar / Lemon Squeezy",
                priority="P1",
                category="billing",
                status="configured"
                if any([s.request_finance_invoice_base_url, s.polar_checkout_base_url, s.lemon_squeezy_checkout_base_url])
                else "disabled",
                free_mode="Без месячной оплаты; комиссии появляются только после продаж.",
                used_by=["checkout intents", "one-shot invoices"],
                env_vars=[
                    "WR3_REQUEST_FINANCE_INVOICE_BASE_URL",
                    "WR3_POLAR_CHECKOUT_BASE_URL",
                    "WR3_LEMON_SQUEEZY_CHECKOUT_BASE_URL",
                ],
                next_step="Локально держать mock; первого провайдера подключать только при старте payment ops.",
                notes=["Request/Polar/Lemon требуют аккаунт и принятие условий."],
            ),
            IntegrationDescriptor(
                id="ton_connect",
                label="TON Connect",
                priority="P1",
                category="telegram_payments",
                status="configured" if s.ton_connect_manifest_url else "disabled",
                free_mode="У SDK нет фиксированной platform fee; комиссии wallet/network возможны.",
                used_by=["будущие wallet-платежи в Mini App"],
                env_vars=["WR3_TON_CONNECT_MANIFEST_URL"],
                next_step="Добавлять manifest только после готовности billing UX.",
                notes=["MVP держит TON как placeholder, не Telegram Stars."],
            ),
            IntegrationDescriptor(
                id="defillama_hacks",
